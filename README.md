...
* 进行分组，每组原则上不能超过4个人，选出一名组长
* 组长在自己的项目中搭建开发环境，复制 ```gameengine``` 文件夹和 ```start.js```文件
* 确认可以通过 ```node start.js gameengine``` 运行项目
* 在 gameengine文件夹中编写一个 ```README.md```文件，并确保前五行是以下格式：
...
> 注：老师会根据这个 README.md 文件进行代码扫描，提取每个组的组员信息，所以请务必按照此格式来撰写


#### 作业要求

* 实现一个网格地图游戏。
    * 创建一个新文件夹，名为 game，这个游戏会在浏览器中运行，而非 Electron 中。
    * 读取 ```map.json```配置文件，并应用 AStar 算法进行寻路。
    * 读取 ```map.json```的方法，请百度 ``` XMLHttpRequest ```
    * 人物会在场景中走动，点击哪个网格，就会移动到这个网格上。
    * 游戏界面参考参考《魔塔》，但是用鼠标交互，而非键盘交互。
* 实现一个网格地图编辑器
    * 以 gameengine 文件夹中的样例代码为模板
    * 编写编辑器UI，必须实现以下功能
        * 一个属性编辑面板
        * 点击任意网格，属性面板中显示这个网格的属性，包括
            ...
            * 网格素材
        * 至少包含6种网格素材
        * “是否可以走”和“网格素材”需要实现一个单选按钮，必须选中、不选中两种状态
        * 保存和撤销功能
 * 保存后的配置文件应该可以拷贝到 ```game```文件夹中并直接使用
 
 #### 建议
 
 四位同学的建议分工为：
 * 一人完成游戏本身的开发，以及地图模块
 * 一人完成地图编辑器 UI 组件的编写（如单选按钮，事件机制完善）
 * 一人完善编辑器的整体逻辑（如保存撤销，整体调用关系等）
 * 一人提供美术素材，包括人物、场景和界面编辑器
 
每位同学都必须在 GitHub 上提交代码，包括美术素材的同学要必须使用 SourceTree 提交
严禁使用U盘进行版本控制，如果组长的 repo 提交日志里没有某位组员的提交日志，该组员以本作业零分处理

由于本次作业比较复杂，请同学们在周日晚上之前尽可能完成大部分内容，在周日晚上主要负责答疑，而非完成作业本身。
